...
func newDeliveryZoneRow(t tr, z orders.DeliveryZone) deliveryZoneRow {
	...
	if z.IsActive {
		row.ChipLabel, row.ChipClass, row.ToggleLabel = t.T("admin.points.active"), "admin-chip--active", t.T("admin.common.deactivate")
	} else {
		row.ChipLabel, row.ChipClass, row.ToggleLabel = t.T("admin.points.inactive"), "admin-chip--inactive", t.T("admin.common.activate")
	}
	return row
}
...
func (p *deliveryPage) render(w http.ResponseWriter, r *http.Request, errMsg string) {
	...
	if err != nil {
		http.Error(w, p.h.tr(r).T("admin.delivery.load_failed"), http.StatusInternalServerError)
		return
	}
	data := deliveryPageData{Error: errMsg, FlatFee: zoneSom(orders.CurrentSettings().DeliveryFee)}
	for _, z := range zones {
		data.Rows = append(data.Rows, newDeliveryZoneRow(p.h.tr(r), z))
		data.AnyActive = data.AnyActive || z.IsActive
	}
	page := p.h.shellPageData("delivery", "admin.nav.delivery", st)
	page.Data = data
	if errMsg != "" {
		w.WriteHeader(http.StatusBadRequest)
	}
	if err := p.h.render.Render(w, "delivery", page); err != nil {
		http.Error(w, p.h.tr(r).T("admin.err.render"), http.StatusInternalServerError)
	}
}
...
func parseSom(s string) (v float64, present bool, err error) {
	...
	if err != nil || math.IsNaN(v) || math.IsInf(v, 0) {
		return 0, true, localizedError{"admin.delivery.err_amount"}
	}
	return v, true, nil
}
...
func parseZoneForm(r *http.Request, active bool) (orders.DeliveryZoneInput, error) {
	...
	if !ok {
		return in, localizedError{"admin.delivery.err_fee_required"}
	}
	in.Fee = fee
	...
	if s := strings.TrimSpace(r.FormValue("sort_order")); s != "" {
		...
		if err != nil || n < -1000000 || n > 1000000 {
			return in, localizedError{"admin.delivery.err_sort_order"}
		}
		in.SortOrder = n
	}
	return in, nil
...
func (p *deliveryPage) create(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		p.render(w, r, p.h.tr(r).T("admin.err.form"))
		return
	}
	...
	if err != nil {
		p.render(w, r, appErrMessage(p.h.tr(r), err))
		return
	}
