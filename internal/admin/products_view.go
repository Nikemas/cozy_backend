...
func resolveProductsPoint(pts []*points.Point, requested string, oos bool) productsPointSel {
	...
	sel.OOS = oos && sel.ID != ""
	sel.Options = append(sel.Options, PointOptionVM{ID: "", Name: "Все точки (без колонки)", Selected: sel.ID == ""})
	for _, p := range pts {
		name := p.Name
		if !p.IsActive {
			name += " (неактивна)"
