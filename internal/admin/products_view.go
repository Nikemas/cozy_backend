...
// pluralRu picks the Russian plural form for n, mirroring the design
// canvas's plural(n, forms) helper (~838-844): forms[0] for 1 (but not
// 11), forms[1] for 2-4 (but not 12-14), forms[2] otherwise.
...
// findCategoryByID searches the full tree the same way findCategoryBySlug
// does, but by id — used to resolve a product's stored category_id back
// into a display path/selects when rendering the edit form.
func findCategoryByID(tree []*catalog.Category, id string) *catalog.Category {
	for _, c := range tree {
		if c.ID == id {
			return c
		}
		if found := findCategoryByID(c.Children, id); found != nil {
			return found
		}
	}
	return nil
