...
func (h *handlers) productsListPage(w http.ResponseWriter, r *http.Request) {
	...
	pointSel := resolveProductsPoint(h.tr(r), pts, query.Get("point"), query.Get("oos") == "1")
	...
	for i, p := range products {
		...
		if pointStock != nil {
			rows[i].PointStockLabel, rows[i].PointStockFG, rows[i].PointStockBG = stockChip(h.tr(r), pointStock[p.ID])
		}
	}
