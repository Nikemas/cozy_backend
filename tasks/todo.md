...
- [x] Полный обход вживую: обнаружился локальный `postgresql@16` (brew service) с уже смигрированной БД `cozy_dev` (поднята другой сессией) — собрал бинарь с текущего `main`, поднял на отдельном порту `:8081` (не трогая чужой процесс на `:8080`), завёл одноразовый тестовый `owner`-аккаунт, прошёл через браузер (`mcp__Claude_Browser`) все экраны: Вход → Заказы (список+детально, реальные заказы из БД) → Товары (список+редактирование, реальные вариации/остатки) → Отчёты (реальные `AggregateSales`/`BrandSales`, bar-chart) → Точки → Сотрудники. Ни одной 500/паники. Инвариант «последний owner» проверен вживую (деактивировал второго owner → попытка деактивировать последнего → корректный баннер «нельзя понизить или деактивировать последнего владельца», отклонено). После проверки: тестовый аккаунт удалён, второй owner возвращён в активное состояние, процесс на `:8081` остановлен — `cozy_dev` в исходном состоянии.
- [x] Визуальная сверка с `Cozy Admin.dc.html` — сайдбар/чипы/таблицы/карточки/чарт визуально совпадают с канвасом; ревью с пользователем — см. скриншоты, отправленные в сессии
...
## Task S: payments + mock Bakai — DONE (кроме реального Bakai)

**Description:** Онлайн-оплата картой (шаг 8 ТЗ, «Bakai: вебхук, привязка оплаты к заказу») без доступа к банку. Новый пакет `internal/payments`: интерфейс `Provider` (`CreatePayment` → `ExternalID`+redirect URL, `ParseCallback` — проверка токена + разбор, `QueryStatus`, `Ready`), `MockProvider` (локальная тестовая страница вместо банка) и `BakaiProvider`-заглушка («не настроено» везде). Выбор — `PAYMENTS_PROVIDER=mock|bakai`. Весь поток `online_card`-заказ → `payment_url` → вебхук → заказ оплачен проходит end-to-end уже сейчас; когда Bakai даст API — переписываются только тела методов `BakaiProvider`.

**Acceptance criteria:**
- [x] Миграция **000021** (`000021_payments_online_card`, номер был свободен и в worktree, и в `main` на момент ребейза). Таблица `payments` уже существовала (000012) — не создаётся заново, а расширяется: `currency TEXT NOT NULL DEFAULT 'KGS'` + CHECK, уникальный индекс `(provider, provider_tx_id)` (идемпотентность вебхуков; `provider_tx_id` из ТЗ и есть «external_id»), индекс по `order_id`; в enum `payment_status` добавлен `cancelled` (тип пересоздаётся, чтобы down мог его убрать); `payment_method` — это ENUM, в нём уже было неиспользуемое `'online'` → `RENAME VALUE 'online' TO 'online_card'` (транзакционно и обратимо, в отличие от `ADD VALUE`); `orders.payment_status payment_status NULL` + CHECK «задан ⇔ online_card» (+ backfill для повторного up после down). Down полностью обратный (`cancelled` сворачивается в `failed`)
- [x] `POST /api/v1/orders` принимает `payment_method` (`cash_on_delivery` по умолчанию — старое поведение, `online_card`, прочее → 400 `invalid_payment_method`). `online_card`: заказ + резерв остатков + `payments` (pending) в одной транзакции (`orders.CreateOnlineOrder`), затем сессия у провайдера, `provider_tx_id` сохраняется, в ответе `payment_url` и `payment_status: pending`
- [x] `POST /api/v1/payments/{provider}/callback` (+ алиас из ТЗ `POST /api/v1/payments/bakai/webhook`): токен в заголовке `X-Webhook-Token` (ТЗ §10), строка `payments` под `FOR UPDATE`, обновление `payments` + `orders.payment_status` (+ отмена заказа и возврат остатков при неуспехе) — одна транзакция через `dbtx.WithTx`; повтор того же события — no-op (`applied:false`), сумма `paid` сверяется до тыйына (`amount_mismatch`), противоречивые события (paid после failed и т.п.) логируются ERROR и не применяются (200, чтобы банк не ретраил), `raw_webhook` сохраняется
- [x] Mock: `GET/POST /api/v1/payments/mock/checkout/{externalId}` — HTML-страница «Оплатить / Отказ банка / Отменить оплату»; POST подписывает callback mock-токеном и прогоняет его через тот же `HandleCallback`, что и настоящий вебхук. Маршруты регистрируются **только** когда провайдер = mock (иначе 404). Скриптуется: `curl -X POST "$payment_url?result=paid"`
- [x] `bakai`-заглушка: `Ready()`/`CreatePayment`/`QueryStatus` → 503 `payments_not_configured` («онлайн-оплата пока недоступна, выберите оплату при получении»); `online_card`-заказ отклоняется **до** создания заказа (остатки не трогаются); вебхук сначала проверяет токен (401), потом 503
- [x] Остатки — решение задокументировано в doc-комментарии `payments.Service`: резерв при создании заказа (как у COD, `FOR UPDATE`), `paid` остатки не трогает, `failed`/`cancelled`/ошибка провайдера при открытии сессии → заказ `cancelled` + возврат остатков на `point_id` заказа (`orders.CancelUnpaidOrderTx`, upsert в `stock`, тот же порядок блокировок по variant_id) — только если заказ ещё `placed`; повторный callback не может вернуть остатки дважды
- [x] Админка: метка оплаты в списке/карточке заказа учитывает статус («Онлайн, оплачено / ожидает оплаты / оплата не прошла / оплата отменена / возврат»)
- [x] `openapi.yaml` (0.6.0): `payment_method`/`payment_status`/`payment_url`, `OrderCreateResponse`, 502/503 для `POST /orders`, тег Payments, три пути, схема `webhookToken`
- [x] `.env.example`: `PAYMENTS_PROVIDER`, уточнён `PUBLIC_BASE_URL`

**Verification:**
- [x] `gofmt -l .` — чисто; `go build ./...`, `go vet ./...` — чисто; `go test ./...` — все пакеты `ok`; `golangci-lint run ./...` — 0 issues
- [x] Новые тесты (sqlmock/фейки): `internal/payments/service_test.go` (таблица переходов, mock/bakai провайдеры, `HandleCallback` paid/duplicate/failed→restock/amount mismatch/unknown/wrong provider, `PlaceOnlineOrder` happy/not-ready/компенсация при сбое провайдера), `internal/orders/payment_test.go` (`CreateOnlineOrder` пишет pending-платёж в той же транзакции, `CancelUnpaidOrderTx` — порядок variant_id и no-op для не-`placed`), `internal/httpapi/payments_test.go` (хендлеры заказа/вебхука/mock-страницы, регистрация mock-маршрутов только для mock), `internal/config` (провайдер по умолчанию, `PaymentsBaseURL`), `internal/admin` (метки)
- [x] Миграция на **живом** Postgres 16 (временный локальный кластер Homebrew в scratchpad, не docker): все 21 up с нуля → проверка enum/колонок/индексов/CHECK → down 000021 (с данными `cancelled` и `online_card`) → up снова — чисто
- [x] End-to-end на том же Postgres с собранным сервером (`PAYMENTS_PROVIDER=mock`, `SMS_MOCK_OTP`): вход по OTP → `online_card`-заказ (остаток 5→3) → mock-страница → paid (заказ `placed/paid`, остаток 3, `raw_webhook` записан) → повтор и противоречивый failed — без изменений; второй заказ → failed (заказ `cancelled/failed`, остаток вернулся 1→3, повтор cancelled — без повторного возврата); прямой вебхук с неверной суммой → 400 `amount_mismatch`, с верной → 200 `applied:true`; неверный/пустой токен → 401; чужой провайдер → 404; COD-заказ без `payment_status`/`payment_url`. С `PAYMENTS_PROVIDER=bakai`: `online_card` → 503, заказов/остатков не прибавилось, mock-страница 404, вебхук 401/503
- [ ] Настоящий Bakai — нет доступа к API банка (формат запросов/подписи неизвестен), `BakaiProvider` — заглушка
- [ ] Мобилка (`cozy_mobile`) и сайт не тронуты: сайт-чекаут по-прежнему только COD; приложению нужно отправлять `payment_method`, открывать `payment_url` и опрашивать `GET /api/v1/orders/{id}`
- [ ] Админ-страница заказа с новой меткой не открывалась в браузере (проверено только юнит-тестом `paymentLabel`)

**Files touched:** `migrations/000021_payments_online_card.{up,down}.sql`; `internal/payments/{provider,mock,bakai,service,service_test}.go` (новый пакет); `internal/orders/payment.go` + `payment_test.go` (новые: `PaymentStatus`, `CreateOnlineOrder`, `CancelUnpaidOrderTx`), `internal/orders/order.go` (`PaymentOnline` → `PaymentOnlineCard`, поле `PaymentStatus`, `CreateOrder` → общий `createOrder` с хуком внутри транзакции, `payment_status` в SELECT/INSERT), `internal/orders/admin.go`, `internal/reports/repo.go` (колонка в SELECT), `internal/orders/notifier_test.go` (колонка в фикстурах); `internal/httpapi/payments.go` + `payments_test.go` (новые), `internal/httpapi/orders.go` (+ `payment_method`, `payment_url`; `RegisterOrderRoutes` принимает `*orders.Service` и `*payments.Service`), `orders_test.go`; `internal/config/config.go` (`PaymentsProvider`, `PaymentsBaseURL()`, `PUBLIC_BASE_URL` обрезается от `/`) + тесты; `cmd/server/main.go`; `internal/admin/orders.go` + тест; `internal/notifications/messages.go` (переименованная константа, «онлайн картой»); `openapi.yaml`; `.env.example`

**Design decisions:**
- **Провайдер по умолчанию: `mock` вне prod, `bakai` при `APP_ENV=prod`** (отклонение от «default mock» в брифе). Mock-страница позволяет «оплатить» любой заказ бесплатно; на живом магазине незаданная переменная не должна это включать. Staging может явно выставить `PAYMENTS_PROVIDER=mock` — тогда в лог пишется WARN. Отказ стартовать (как у `SMS_MOCK_OTP`) не сделан сознательно: staging, скорее всего, крутится с `APP_ENV=prod`, и автодеплой упал бы.
- **`PaymentStatus` живёт в `internal/orders`**, `payments.Status` — алиас: `Order` несёт это значение, а `payments` уже импортирует `orders` (обратный импорт дал бы цикл).
- **`orders.payment_status` денормализован** (а не вычисляется из последнего `payments`), чтобы список/карточка заказа и мобилка видели статус без JOIN; обновляется в той же транзакции, что и `payments`.
- **Неуспешная оплата отменяет заказ**, а не оставляет его ждать повторной оплаты: иначе резерв висел бы бесконечно (джобы истечения нет). Повторить = оформить новый заказ.
- **Ошибка провайдера при открытии сессии** → компенсирующая транзакция (payment `failed`, заказ `cancelled`, остатки назад) на `context.WithoutCancel`, клиенту 502 `payment_create_failed`.
- **Ребейз на свежий `main`** (Task T/R смержены параллельно): конфликт `PublicBaseURL` решён в пользу одного поля с семантикой Task T (пусто = без ссылок) + `Config.PaymentsBaseURL()` с фолбэком на `http://localhost<порт>`.

**Follow-ups (не сделано):**
- [ ] Истечение брошенных `pending`-оплат (джоба: `QueryStatus` у провайдера / отмена через N минут с возвратом остатков) — сейчас резерв держится, пока персонал не отменит заказ
- [ ] Отмена заказа из админки (`AdminUpdateStatus` → `cancelled`) не возвращает остатки и не трогает оплату — ни для COD, ни для online; оплаченный и затем отменённый заказ требует ручного возврата (при `paid`-вебхуке на уже отменённый заказ пишется ERROR «refund needed»)
- [ ] Автоотмена заказа из-за неуспешной оплаты не шлёт push покупателю (`CancelUnpaidOrderTx` — функция пакета, мимо `Service.notifyStatusChanged`); Telegram-оповещение персоналу о новом `online_card`-заказе уходит сразу при создании, до оплаты
- [ ] Отчёты по продажам считают `online_card`-заказы в статусе `placed` независимо от `payment_status`
- [ ] Реальный `BakaiProvider` + продовый `PUBLIC_BASE_URL`, проверка подписи по спецификации банка
