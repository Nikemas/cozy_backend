...
## Wave 5: Нормализация изображений товаров (`internal/media`)

### Overview

Найдено 2026-09-23 при рестайле карточки товара в мобилке. Новый дизайн (design-system §4.2) ставит фото в белый квадратный колодец 1:1 с `BoxFit.contain` — геометрия карточки жёсткая, но **содержимое** фото ничем не нормализуется: бэкенд сейчас не трогает загружаемый файл вообще. `POST /admin/api/media/presign-upload` (Task E) отдаёт presigned PUT прямо в MinIO, файл идёт мимо приложения, проверяется только *заявленный* клиентом `content_type` (`internal/media/objectkey.go`). Следствия: фото на сером/цветном фоне встаёт прямоугольником на белом колодце; обувь у разных товаров снята в разном масштабе и кадре; нет лимита веса (8-МБ JPEG поедет в сетку как есть); нет превью — один и тот же файл грузится и в сетку 2 колонки, и на экран товара.

Важный факт, который делает задачу дешёвой сейчас: `presign-upload` **ещё никем не вызывается** — grep по репозиторию находит его только в `internal/media/routes.go`, форма загрузки в админке (Wave 4 Task 2) ещё не написана. Значит контракт можно заменить без миграции клиентов; чем позже — тем дороже.

### Architecture Decisions

- **Загрузка через приложение, а не presigned PUT.** Нормализовать файл можно либо принимая его сервером, либо асинхронно до-обрабатывая уже залитый объект. Второй путь требует очереди/воркера, которых в проекте нет, и оставляет окно, когда в каталоге лежит необработанное фото. Выбирается первый: `POST /admin/api/media/upload` (multipart), сервер обрабатывает синхронно и кладёт в MinIO уже готовые варианты. Для 2–5 точек продаж и редких загрузок пропускная способность не проблема. `presign-upload` удаляется (вызывающих нет), `Client.PresignPut` остаётся только если понадобится ещё где-то.
- **Реальный тип определяется по сигнатуре файла**, а не по `Content-Type` из запроса. Текущая проверка доверяет клиенту; при серверной загрузке это лечится бесплатно (`http.DetectContentType` / декодирование).
- **Нормализуем в JPEG на белом фоне, не в WebP.** Прозрачность в карточке не нужна: колодец в сетке белый и фон экрана товара белый (design-system §4.2, §4.7) — заливка белым визуально бесшовна и как раз даёт ту однородность, ради которой задача заводится. JPEG-энкодер есть в stdlib; pure-Go WebP-энкодер либо тянет cgo, либо молодой — не тот риск, который стоит брать ради ~25% веса. Пересмотреть, если появится тёмная тема (тогда прозрачный PNG/WebP станет обязательным).
- **Два варианта размера на фото**: `full` 1200×1200 (экран товара) и `thumb` 400×400 (сетка 2 колонки на 3x-экране). Обувь вписывается в квадрат с полями ~8%, как в дизайне.
- **Ключи объектов**: `products/<uuid>/full.jpg` и `products/<uuid>/thumb.jpg`. `product_images.object_key` продолжает хранить **один** ключ (full) — колонок не добавляем; thumb выводится заменой суффикса по документированному соглашению. Старые ключи вида `products/<uuid>.jpg` (без вариантов) остаются валидными и отдаются как есть — никакой миграции существующих данных.

### Task List

#### Task R: Серверная нормализация загружаемых фото товаров — DONE (manual не прогнан, см. `todo.md`)

**Description:** Заменить presigned-upload на загрузку через приложение с нормализацией изображения: валидация по сигнатуре, применение EXIF-ориентации, вырезание метаданных, вписывание в квадрат с белым фоном и полями, генерация `full`/`thumb`, заливка обоих в MinIO, возврат `object_key` для `product_images`.

**Acceptance criteria:**
- [x] `POST /admin/api/media/upload` (multipart/form-data, роль owner/manager через тот же `staffSvc.RequireRole`, что и presign) принимает файл и возвращает `{"object_key": "products/<uuid>/full.jpg"}`
- [x] Лимит тела запроса 10 МБ — превышение даёт `apperr.BadRequest`, а не 500 и не OOM (`http.MaxBytesReader`)
- [x] Реальный формат определяется декодированием, а не заголовком; допускаются JPEG/PNG/WebP на входе, всё остальное — 400 с понятным русским сообщением (как в `extensionForContentType`)
- [x] Минимальное разрешение — отказ, если меньшая сторона < 600 px (апскейл мыла в карточку не пускаем); сообщение объясняет требование
- [x] EXIF-ориентация применяется до ресайза (фото с телефона вверх ногами — частый случай); метаданные в результат не попадают
- [x] Результат: квадрат 1:1, изображение вписано (`contain`) с полями ~8% от стороны, свободное место залито белым; `full` 1200×1200, `thumb` 400×400, JPEG quality 85
- [x] Оба варианта заливаются в MinIO под `products/<uuid>/{full,thumb}.jpg`; при ошибке заливки второго — первый удаляется (не оставляем половину)
- [x] `POST /admin/api/media/presign-upload` удалён; `openapi.yaml` обновлён (новый эндпоинт вместо старого, описано соглашение `full`/`thumb`)
- [x] Существующие ключи `products/<uuid>.<ext>` продолжают отдаваться без изменений — правило «thumb = замена суффикса» применяется только к ключам, оканчивающимся на `/full.jpg`; задокументировано там же, где строятся URL

**Verification:**
- [x] `gofmt -l .`, `go build ./...`, `go vet ./...`, `go test ./...`, `golangci-lint run ./...` — чисто
- [x] Юнит-тесты на чистую функцию нормализации (вход — сгенерированные в тесте изображения, без сетевых и MinIO-зависимостей): широкое фото, высокое фото, ровный квадрат, PNG с прозрачностью → белый фон, EXIF-поворот, слишком маленькое → ошибка, не-изображение → ошибка, размеры и формат результата
- [x] Тест на лимит тела (файл > 10 МБ → 400, не паника)
- [ ] Manual: загрузить через curl 3–4 реальных фото обуви с разным фоном и кадром, скачать `thumb` и глазами проверить, что они встают в ряд одинаково (локального Postgres/MinIO на машине разработки может не быть — тогда на staging)

**Dependencies:** Task E (MinIO `Client`, бакет, `PresignGet` для чтения). Блокирует Wave 4 Task 2 в части формы загрузки фото — форму писать уже под новый эндпоинт, старый не трогать.

**Files touched (план):**
- `internal/media/normalize.go` + `normalize_test.go` — новая чистая логика (decode → EXIF → contain-в-квадрат → белый фон → encode), без знания про HTTP и MinIO
- `internal/media/routes.go` — новый multipart-хендлер вместо presign; `internal/media/objectkey.go` — ключи вариантов
- `internal/media/client.go` — метод заливки объекта из памяти (сейчас есть только presign)
- `openapi.yaml` — эндпоинт и соглашение об именовании вариантов
- `internal/web/catalog_view.go` — `photoURL` должен уметь отдавать `thumb` для сеточных превью витрины
- Мобилка (`cozy_mobile`, отдельной задачей): сборка URL под `thumb` в `ProductCard` и `full` на экране товара

**Design decisions:** Зависимости — `golang.org/x/image/draw` (качественный ресемплинг, `CatmullRom`) и что-то минимальное для EXIF-ориентации; `image/jpeg`, `image/png` из stdlib. Новых cgo-зависимостей не тянуть — сборка образа в `docker/` рассчитана на статический бинарь.

### Risks and Mitigations (Wave 5)

| Risk | Impact | Mitigation |
|---|---|---|
| Загрузка через приложение упирается в таймауты/память при больших файлах | Низкий (редкие загрузки, 2–5 точек) | `MaxBytesReader` 10 МБ + декодирование в стриме; не оптимизировать заранее |
| Белый фон испортит фото, снятые на тёмном фоне | Средний (визуальный) | Это и есть цель задачи — привести всё к белому; в форме админки дать превью результата до сохранения |
| Соглашение «thumb = замена суффикса» разъедется между Go-витриной и мобилкой | Средний | Задокументировать в `openapi.yaml` и продублировать в `design-system`/README мобилки; в обоих местах — один хелпер, а не инлайн-строки |
| Старые фото остаются ненормализованными | Низкий (каталог ещё не наполнен) | Ничего не мигрируем; при необходимости — разовый скрипт в `scripts/`, отдельной задачей |

### Open Questions (Wave 5)

- Поля 8% — жёстко в коде или настройка? Пока жёстко: значение взято из design-system §4.2 (фото 88% сцены на экране товара), менять его без правки дизайна смысла нет.
- Нужен ли третий вариант размера под баннеры/«Может понравиться»? Сейчас нет — две сетки используют одинаковую ширину плитки.
- Требовать ли от контент-менеджера прозрачный PNG (и тогда белый фон — только фоллбэк), или сознательно сводить всё к белому JPEG? Решение выше — второе; пересмотреть при появлении тёмной темы.
