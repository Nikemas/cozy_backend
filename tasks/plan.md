...
#### Task R: Серверная нормализация загружаемых фото товаров
...
**Acceptance criteria:**
- [ ] `POST /admin/api/media/upload` (multipart/form-data, роль owner/manager через тот же `staffSvc.RequireRole`, что и presign) принимает файл и возвращает `{"object_key": "products/<uuid>/full.jpg"}`
- [ ] Лимит тела запроса 10 МБ — превышение даёт `apperr.BadRequest`, а не 500 и не OOM (`http.MaxBytesReader`)
- [ ] Реальный формат определяется декодированием, а не заголовком; допускаются JPEG/PNG/WebP на входе, всё остальное — 400 с понятным русским сообщением (как в `extensionForContentType`)
- [ ] Минимальное разрешение — отказ, если меньшая сторона < 600 px (апскейл мыла в карточку не пускаем); сообщение объясняет требование
- [ ] EXIF-ориентация применяется до ресайза (фото с телефона вверх ногами — частый случай); метаданные в результат не попадают
- [ ] Результат: квадрат 1:1, изображение вписано (`contain`) с полями ~8% от стороны, свободное место залито белым; `full` 1200×1200, `thumb` 400×400, JPEG quality 85
- [ ] Оба варианта заливаются в MinIO под `products/<uuid>/{full,thumb}.jpg`; при ошибке заливки второго — первый удаляется (не оставляем половину)
- [ ] `POST /admin/api/media/presign-upload` удалён; `openapi.yaml` обновлён (новый эндпоинт вместо старого, описано соглашение `full`/`thumb`)
- [ ] Существующие ключи `products/<uuid>.<ext>` продолжают отдаваться без изменений — правило «thumb = замена суффикса» применяется только к ключам, оканчивающимся на `/full.jpg`; задокументировано там же, где строятся URL

**Verification:**
- [ ] `gofmt -l .`, `go build ./...`, `go vet ./...`, `go test ./...`, `golangci-lint run ./...` — чисто
- [ ] Юнит-тесты на чистую функцию нормализации (вход — сгенерированные в тесте изображения, без сетевых и MinIO-зависимостей): широкое фото, высокое фото, ровный квадрат, PNG с прозрачностью → белый фон, EXIF-поворот, слишком маленькое → ошибка, не-изображение → ошибка, размеры и формат результата
- [ ] Тест на лимит тела (файл > 10 МБ → 400, не паника)
